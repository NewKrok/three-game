...
const create = ({
  world: { getModule },
  unit,
  config: { id, radius = 0.33, height = 1.8, position = new THREE.Vector3() },
}) => {
  ...
  const updateModelPosition = () => {
    ...
    properties.capsule.collider.getCenter(unit.container.position);
    unit.container.position.y -= height / 2 + radius / 2;
  };
  ...
  const teleportTo = (position) => {
    properties.capsule.collider.translate(
      position.clone().sub(unit.container.position)
    );
  };
